...
# ══════════════════════════════════════════════════════════════════
#  EVENTOS INTERESTELARES
#  Ocurren durante el trayecto entre sistemas; son deterministas:
#  la misma semilla de origen + destino siempre produce los mismos
#  eventos en el mismo orden.
# ══════════════════════════════════════════════════════════════════

@dataclass
class EventoInterestelar:
    nombre:      str
    categoria:   str   # "Astrofísico" | "Teórico" | "Anómalo"
    emoji:       str
    descripcion: str
    consecuencia: str
    severidad:   str   # "Leve" | "Moderado" | "Crítico"


CATALOGO_EVENTOS: list[EventoInterestelar] = [

    # ── 1. Fenómenos Astrofísicos Reales ────────────────────────────
    EventoInterestelar(
        nombre      = "Planeta Errante",
        categoria   = "Astrofísico",
        emoji       = "🌑",
        descripcion = (
            "Los sensores infrarrojos detectan un cuerpo planetario masivo que no orbita "
            "ninguna estrella. Avanza silencioso en la oscuridad interestelar, detectable "
            "solo a corta distancia por efecto de microlente gravitacional."
        ),
        consecuencia = (
            "Desviación de trayectoria mínima ejecutada. Escáneres de recursos activados: "
            "posible extracción de hidrógeno congelado en superficie. Sin riesgo de colisión."
        ),
        severidad   = "Leve",
    ),
    EventoInterestelar(
        nombre      = "Enjambre de Polvo Interestelar",
        categoria   = "Astrofísico",
        emoji       = "🌫️",
        descripcion = (
            "La nave atraviesa una región con concentración anómala de polvo y "
            "micrometeoritos. A velocidades relativistas, la fricción genera calor "
            "significativo en el escudo frontal y erosión progresiva del casco."
        ),
        consecuencia = (
            "Escudos deflectores activados al 78 %. Velocidad reducida un 12 % para "
            "minimizar erosión estructural. ETA al destino retrasado ligeramente."
        ),
        severidad   = "Moderado",
    ),
    EventoInterestelar(
        nombre      = "Ráfaga de Rayos Gamma (GRB)",
        categoria   = "Astrofísico",
        emoji       = "💥",
        descripcion = (
            "Un pulso de radiación ionizante procedente del colapso de una hipernova "
            "lejana barre el espacio. El frente de radiación es breve pero devastador "
            "para toda la electrónica no blindada expuesta al exterior."
        ),
        consecuencia = (
            "Fallos en cascada en sistemas no blindados. La tripulación se refugia en el "
            "núcleo de radiación mínima durante 4.2 h hasta que el pulso cede. "
            "Diagnósticos en curso."
        ),
        severidad   = "Crítico",
    ),
    EventoInterestelar(
        nombre      = "Nube de Oort Densa",
        categoria   = "Astrofísico",
        emoji       = "☄️",
        descripcion = (
            "Al aproximarse al sistema de destino, la nave penetra en una nube de Oort "
            "inusualmente poblada: cometas inactivos y cuerpos de hielo flotan en todas "
            "las direcciones con densidad superior a la proyectada."
        ),
        consecuencia = (
            "Sistema de maniobras evasivas automáticas activado. "
            "43 evasiones registradas en las últimas 6 horas de navegación. "
            "Sin impactos directos reportados."
        ),
        severidad   = "Moderado",
    ),
    ...
    EventoInterestelar(
        nombre      = "Cerebro de Boltzmann",
        categoria   = "Teórico",
        emoji       = "🧠",
        descripcion = (
            "Una entidad autoconsciente formada espontáneamente por fluctuaciones "
            "cuánticas en el vacío espacial interfiere con los sensores de la nave. "
            "Transmite paradojas matemáticas y lógica no euclidiana antes de disiparse "
            "por entropía."
        ),
        consecuencia = (
            "Interferencia en sensores durante 17 minutos. Los registros contienen "
            "transmisiones codificadas que el ordenador central aún no ha podido "
            "descifrar. La entidad se disolvió sin dejar rastro bariónico."
        ),
        severidad   = "Leve",
    ),
    EventoInterestelar(
        ...
        emoji       = "⚫",
        descripcion = (
            "Una distorsión gravitacional del tamaño de una pelota de golf — remanente "
            "del Big Bang — cruza la trayectoria de la nave. No emite radiación Hawking "
            "detectable a tiempo; solo sus efectos gravitacionales son perceptibles."
        ),
        consecuencia = (
            "Trayectoria desviada 0.003° de forma brusca. Los cálculos de navegación "
            "deben recalcularse desde cero. Sin daños estructurales reportados. "
            "Posición reconfirmada por triangulación de púlsares."
        ),
        severidad   = "Moderado",
    ),
    EventoInterestelar(
        nombre      = "Cuerda Cósmica",
        categoria   = "Teórico",
        emoji       = "〰️",
        descripcion = (
            "Un defecto topológico unidimensional en el tejido del espacio-tiempo cruza "
            "el campo visual de la nave. Las estrellas de fondo se duplican visualmente "
            "por efecto de lente gravitacional extremo, confundiendo los sistemas de "
            "navegación por inercia."
        ),
        consecuencia = (
            "Cálculos del motor de curvatura invalidados temporalmente. Se requieren "
            "2.5 horas para recalibrar el sistema de navegación relativista. "
            "No se registran daños materiales."
        ),
        severidad   = "Moderado",
    ),
    EventoInterestelar(
        nombre      = "Condensado de Materia Oscura",
        categoria   = "Teórico",
        emoji       = "🌀",
        descripcion = (
            "La nave queda atrapada temporalmente en un pozo gravitacional que no emite "
            "ni interactúa con el electromagnetismo. Los sensores no detectan masa "
            "bariónica alguna, pero la inercia de la nave aumenta de forma inexplicable."
        ),
        consecuencia = (
            "Resistencia inercial anómala durante 38 minutos. Los motores compensan "
            "automáticamente aumentando el empuje un 34 %. El fenómeno se disipa sin "
            "daños estructurales ni pérdida de combustible crítica."
        ),
        severidad   = "Leve",
    ),
    ...
    EventoInterestelar(
        nombre      = "Eco de Taquiones",
        categoria   = "Anómalo",
        emoji       = "⏱️",
        descripcion = (
            "Los sensores detectan partículas superlumínicas que viajan hacia atrás "
            "en el tiempo. La nave registra el daño de un impacto y una falla de "
            "sistemas varios minutos antes de que el evento físico ocurra realmente."
        ),
        consecuencia = (
            "Escudos reforzados en sector 7-F antes del impacto real. Gracias a la "
            "anomalía causal, los daños se reducen en un 94 %. Los registros de "
            "la paradoja temporal han sido clasificados."
        ),
        severidad   = "Moderado",
    ),
    EventoInterestelar(
        nombre      = "Enjambre de Sondas Von Neumann",
        categoria   = "Anómalo",
        emoji       = "🤖",
        descripcion = (
            "Millones de máquinas autorreplicantes de origen desconocido desmantelan "
            "metódicamente un asteroide errante cercano. Ignoran por completo la "
            "presencia de la nave… a menos que se interfiera con su ciclo de "
            "recolección de recursos."
        ),
        consecuencia = (
            "La nave mantiene distancia prudencial y silencio de emisiones. Las sondas "
            "continúan su tarea sin reaccionar. Origen y propósito: indeterminados. "
            "Nivel tecnológico estimado: Kardashov II-III."
        ),
        severidad   = "Leve",
    ),
    EventoInterestelar(
        nombre      = "Escombros de Megaestructura",
        categoria   = "Anómalo",
        emoji       = "🏗️",
        descripcion = (
            "La nave navega entre los restos de lo que parece ser un cilindro de "
            "O'Neill destruido o un segmento de una antigua Esfera de Dyson. Las "
            "aleaciones encontradas no corresponden a ningún elemento de la tabla "
            "periódica conocida."
        ),
        consecuencia = (
            "Maniobras evasivas menores ejecutadas. Se recogen 3 muestras de material "
            "para análisis. La datación radiométrica sugiere una antigüedad superior "
            "a 2.3 mil millones de años. Constructor: desconocido."
        ),
        severidad   = "Leve",
    ),
    EventoInterestelar(
        nombre      = "Burbuja de Falso Vacío",
        categoria   = "Anómalo",
        emoji       = "🫧",
        descripcion = (
            "Los instrumentos cuánticos alertan sobre la proximidad del decaimiento "
            "del estado de vacío local. El frente de burbuja avanza a velocidad "
            "lumínica, desintegrando la materia a nivel subatómico. No existe "
            "advertencia previa posible."
        ),
        consecuencia = (
            "SALTO DE EMERGENCIA EJECUTADO en 0.003 s. Sin daños estructurales. "
            "Los registros cuánticos estiman una probabilidad de supervivencia de "
            "apenas el 0.7 %. El sistema de alerta temprana ha sido actualizado."
        ),
        severidad   = "Crítico",
    ),
]
...
def _print_wrapped(texto: str, prefijo: str = "     ", ancho_max: int = 62):
    """Imprime un texto largo con saltos de línea automáticos y sangría."""
    palabras  = texto.split()
    linea_act = ""
    primera   = True
    for palabra in palabras:
        if len(linea_act) + len(palabra) + 1 > ancho_max:
            sangria = prefijo if primera else " " * len(prefijo)
            print(f"{sangria}{linea_act.rstrip()}")
            primera   = False
            linea_act = palabra + " "
        else:
            linea_act += palabra + " "
...
def generar_eventos_viaje(
    seed_origen:  str,
    seed_destino: str,
    min_eventos:  int = 0,
    max_eventos:  int = 3,
) -> list[EventoInterestelar]:
    """
    Genera una lista DETERMINISTA de eventos interestelares para el
    trayecto entre seed_origen y seed_destino.

    La semilla del viaje se deriva de AMBAS seeds mediante derivar_rng,
    garantizando que:
      · El mismo par A→B siempre produce los mismos eventos.
      · Cambiar origen o destino cambia los eventos completamente.
      · El resultado es idéntico en cualquier máquina con cualquier
        versión de Python ≥ 3.6 (SHA-256 es estable).
    """
    rng = derivar_rng(seed_origen, "viaje_a", seed_destino)
    n   = rng.randint(min_eventos, max_eventos)
    return rng.sample(CATALOGO_EVENTOS, min(n, len(CATALOGO_EVENTOS)))


def renderizar_viaje(seed_origen: str, seed_destino: str):
    """
    Muestra en consola los eventos interestelares ocurridos durante
    el trayecto de seed_origen hacia seed_destino.
    """
    eventos = generar_eventos_viaje(seed_origen, seed_destino)
    ancho   = 72

    if not eventos:
        print("\n" + "─" * ancho)
        print("  🚀  TRAYECTO INTERESTELAR — Espacio tranquilo, sin incidentes.")
        print("─" * ancho)
        return

    print("\n" + "═" * ancho)
    print(f"  🚀  TRAYECTO: {seed_origen}  →  {seed_destino}".center(ancho))
    n_str = f"  ⚠   {len(eventos)} evento(s) registrado(s) en ruta"
    print(n_str.center(ancho))
    print("═" * ancho)
    ...
    for i, ev in enumerate(eventos, 1):
        cat_i = ICONO_CAT.get(ev.categoria, "❓")
        sev_i = ICONO_SEV.get(ev.severidad, ev.severidad)
        print(f"\n  {ev.emoji}  [{i}/{len(eventos)}] {ev.nombre.upper()}")
        print(f"     Categoría : {cat_i} {ev.categoria:<12}  Severidad : {sev_i}")
        print(f"     {'─' * 62}")
        _print_wrapped(ev.descripcion,  prefijo="     📡  ")
        print()
        _print_wrapped(ev.consecuencia, prefijo="     ⚙   ")
        print()

    print("═" * ancho)
    print(f"  Llegando a {seed_destino} …".center(ancho))
    print("═" * ancho + "\n")
...
if __name__ == "__main__":
    ...
    seed          = "SOL-442"
    seed_anterior = None   # rastrea el sistema del que venimos para el viaje
    ...
    while True:
        # ── Eventos interestelares ────────────────────────────────────
        # Si venimos de un sistema anterior, primero se resuelven los
        # sucesos del trayecto. Si hubo eventos, se muestra SOLO el viaje
        # y se pausa; el jugador presiona Enter para llegar al destino.
        if seed_anterior is not None:
            eventos_ruta = generar_eventos_viaje(seed_anterior, seed)
            if eventos_ruta:
                renderizar_viaje(seed_anterior, seed)
                input("  ▶  Presiona Enter para llegar al sistema destino…\n")
            else:
                print("\n" + "─" * 72)
                print("  🚀  TRAYECTO INTERESTELAR — Espacio tranquilo, sin incidentes.")
                print("─" * 72 + "\n")
        ...
        # Guardar el nombre real del sistema (post-búsqueda en OEC) como
        # origen del próximo viaje, para que los eventos sean reproducibles.
        seed_anterior = sistema.seed
